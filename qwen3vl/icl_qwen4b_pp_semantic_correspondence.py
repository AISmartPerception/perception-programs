...
def build_conversation(sample, semantic_pp):
    system_ = """
        ...
        If all scores are very low in <perceptionprogram>, look at the images and make your best guess. When answering, only output the letter enclosed by parenthesis---i.e. (A)---without any prose.
        Output Rules:
    ...
    """
    icl_example = """
        You are given a source image with a marked reference point (REF), and a target image containing four candidate points labeled A, B, C, and D. 
        A <perceptionprogram> block is provided, listing similarity scores for each candidate with respect to the reference point. 
        Your goal is to decide which candidate point corresponds semantically to the REF.
        
        One demonstrative example of how to use perception program to solve a certain problem is given below.

        Images: (Image 1 with REF, Image 2 with candidates A, B, C, D)
        Perception: In this particular example, candidates C and D have the highest similarity scores according to the perception program. 
        However, the scores are close — and a visual inspection shows that the REF lies on the mid-lower leg (shank) which is point C, not the upper leg joint near the body which is point D. 
        Therefore, candidate C is semantically aligned with the REF, making it the correct answer.
        Answer: (C)
...
def get_batched_perception_programs(idxs, pp_outputs):
    batched_pps = []
    for i in idxs:
...
def run_qwen_pp_batched(output_root,
    ...
    pp_outputs = read_jsonl("/data00/semantic_correspondence/raw_pp_outputs_2.jsonl")
    N = len(dataset)
    ...
    while pending and attempt <= max_tries:
        ...
        for idxs in pbar:
            ...
            pps = get_batched_perception_programs(idxs, pp_outputs)
...
if __name__ == "__main__":
    output_root = f"/data00/semantic_correspondence/open-source-models/qwen4b-{mode.lower()}/pp/"
